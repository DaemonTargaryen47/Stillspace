...
export default function CirclePage() {
  ...
  const handleRemove = async (memberId) => {
    ...
    if (memberToRemove?.inviteCode) {
      const { data: theirRecord } = await supabase.from('users').select('*').eq('invite_code', memberToRemove.inviteCode).single()
      if (theirRecord?.circle_data) {
        const updatedTheirCircle = JSON.parse(theirRecord.circle_data).filter(m => m.inviteCode !== user.inviteCode)
        await supabase.from('users').update({ circle_data: JSON.stringify(updatedTheirCircle), updated_at: new Date().toISOString() }).eq('invite_code', memberToRemove.inviteCode)
      }
    }
  }
    ...
    <div className="page-wrap" style={{ maxWidth: 520, margin: '0 auto', minHeight: '100vh', paddingBottom: 100 }}>
      ...
      <div style={{ padding: '108px 28px 0' }}>
        ...
        <div className="gradient-card" style={{ padding: '24px', borderRadius: 20, marginBottom: 16, position: 'relative', overflow: 'hidden' }}>
          ...
          <p className="section-label" style={{ fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 14 }}>circle capacity</p>
          <div style={{ display: 'flex', gap: 6, marginBottom: 12, flexWrap: 'wrap' }}>
            {[...Array(max)].map((_, i) => {
              const member = circle[i]
              return (
                <div key={i} className={member ? '' : 'dashed-circle'} style={{ width: 36, height: 36, borderRadius: '50%', background: member ? STATUSES[i % STATUSES.length].color : 'transparent', border: member ? 'none' : '2px dashed #d0cdc8', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 12, fontWeight: 500, color: '#ffffff', transition: 'all 0.4s ease', boxShadow: member ? '0 2px 8px rgba(0,0,0,0.12)' : 'none' }}>
                  {member ? (member.initials || '?') : ''}
                </div>
              )
            })}
          </div>
          <p className="text-p" style={{ fontSize: 15 }}>
            <span style={{ fontFamily: 'var(--font-serif)', fontSize: 24 }}>{circle.length}</span>
            <span className="text-m"> of {max} people connected</span>
          </p>
        </div>

        {joinStatus === 'success' && <div className="info-box" style={{ padding: '14px 18px', borderRadius: 12, marginBottom: 14, borderLeft: '3px solid #8a9e8c' }}><p style={{ fontSize: 14, color: '#4a5a4c' }}>Connected. Both circles updated automatically.</p></div>}
        ...
        {joinStatus === 'error:full' && <div style={{ padding: '14px 18px', background: '#fdf0ef', borderRadius: 12, marginBottom: 14, borderLeft: '3px solid #c0392b' }}><p style={{ fontSize: 14, color: '#c0392b' }}>Your circle is full.</p></div>}
        ...
        {!isGuest && (
          <>
            <div className="card" style={{ padding: '20px', borderRadius: 18, marginBottom: 12 }}>
              <p className="section-label" style={{ fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>your invite code</p>
              <p className="text-s" style={{ fontSize: 14, marginBottom: 14, lineHeight: 1.6 }}>Share this with someone. They enter it and both circles update automatically.</p>
              <div style={{ display: 'flex', gap: 10, alignItems: 'center' }}>
                <div className="invite-box" style={{ flex: 1, padding: '12px 16px', borderRadius: 12, fontSize: 20, fontFamily: 'monospace', letterSpacing: '0.2em', textAlign: 'center', fontWeight: 500 }}>{user.inviteCode}</div>
            ...
            <div className="card" style={{ padding: '20px', borderRadius: 18, marginBottom: 24 }}>
              <p className="section-label" style={{ fontSize: 11, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 6 }}>join someone's circle</p>
              <p className="text-s" style={{ fontSize: 14, marginBottom: 14, lineHeight: 1.6 }}>Enter their code. You will both be connected instantly.</p>
              {showJoin ? (
                <div>
                  <div style={{ display: 'flex', gap: 10, alignItems: 'center', marginBottom: 8 }}>
                    <input value={joinCode} onChange={e => setJoinCode(e.target.value.toUpperCase())} placeholder="e.g. QJII79" maxLength={6}
                      ...
                      style={{ flex: 1, padding: '12px 16px', border: '1.5px solid transparent', borderRadius: 12, fontSize: 20, fontFamily: 'monospace', letterSpacing: '0.2em', textAlign: 'center', outline: 'none', transition: 'border-color 0.2s ease', fontWeight: 500 }}
                      onFocus={e => e.target.style.borderColor = '#8a9e8c'}
                      onBlur={e => e.target.style.borderColor = 'transparent'}
                      onKeyDown={e => e.key === 'Enter' && handleJoin()}
                    />
                    <button onClick={handleJoin} disabled={joinCode.length < 4 || loading} style={{ padding: '12px 20px', background: joinCode.length >= 4 && !loading ? '#2c2c2e' : '#f0ede8', color: joinCode.length >= 4 && !loading ? '#faf9f7' : '#b0a99a', border: 'none', borderRadius: 12, fontSize: 14, cursor: joinCode.length >= 4 ? 'pointer' : 'default', whiteSpace: 'nowrap' }}>
                ...
                <button onClick={() => setShowJoin(true)} className="text-s" style={{ padding: '10px 20px', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 999, background: 'none', fontSize: 14, cursor: 'pointer' }}
        ...
        {isGuest ? (
          <>
            <p className="text-s" style={{ fontSize: 14, marginBottom: 14 }}>A preview of how your circle would look.</p>
              ...
              <div key={m.id} style={{ animationDelay: i * 0.05 + 's' }} className="fade-up">
                <div className="card" style={{ display: 'flex', alignItems: 'center', gap: 14, padding: '14px 18px', borderRadius: 16 }}>
                  ...
                  {removeConfirm === m.id ? (
                    ...
                    <button onClick={() => setRemoveConfirm(m.id)} className="text-f" style={{ width: 28, height: 28, border: '1px solid rgba(0,0,0,0.08)', borderRadius: '50%', background: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: 16, flexShrink: 0, transition: 'all 0.2s ease' }}
                    onMouseEnter={e => { e.currentTarget.style.borderColor = '#c0392b'; e.currentTarget.style.color = '#c0392b' }}
                    onMouseLeave={e => { e.currentTarget.style.borderColor = 'rgba(0,0,0,0.08)'; e.currentTarget.style.color = '' }}
                    >×</button>
                  )}
                </div>
              </div>
