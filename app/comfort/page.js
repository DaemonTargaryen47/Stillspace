...
import { rewardFloat, inkBurst, rewardRipple } from '../../lib/microAnimations'
...
const SOUND_GROUPS = [
  ...
  {
    id: 'noise', label: 'Focus Noise', icon: '🌫',
    sounds: [
...
class SoundEngine {
  constructor() { this.ctx = null; this.nodes = {}; this.gainNodes = {}; this.masterGain = null }
  init() {
    if (this.ctx) return
  ...
  setMasterVolume(vol) { if (this.masterGain) this.masterGain.gain.setTargetAtTime(vol, this.ctx.currentTime, 0.1) }
  setSoundVolume(id, vol) { if (this.gainNodes[id]) this.gainNodes[id].gain.setTargetAtTime(vol, this.ctx.currentTime, 0.1) }
  createNoise(type) {
    ...
    const source = this.ctx.createBufferSource(); source.buffer = buffer; source.loop = true; return source
  }
  buildSound(id, freq, variant) {
    const gainNode = this.ctx.createGain(); gainNode.gain.value = 0; gainNode.connect(this.masterGain); this.gainNodes[id] = gainNode
    if (['white','brown','pink'].includes(freq)) {
      const n = this.createNoise(freq); n.connect(gainNode); n.start(); this.nodes[id] = [n]
    } else if (freq === 'rain') {
      const n = this.createNoise('white'); const f = this.ctx.createBiquadFilter(); f.type = 'bandpass'; f.frequency.value = variant === 'heavy' ? 800 : 400; f.Q.value = variant === 'heavy' ? 0.5 : 1; n.connect(f); f.connect(gainNode); n.start(); this.nodes[id] = [n]
    } else if (freq === 'ocean') {
      const n = this.createNoise('pink'); const f = this.ctx.createBiquadFilter(); f.type = 'lowpass'; f.frequency.value = 600; const lfo = this.ctx.createOscillator(); const lg = this.ctx.createGain(); lfo.frequency.value = 0.1; lg.gain.value = 200; lfo.connect(lg); lg.connect(f.frequency); n.connect(f); f.connect(gainNode); n.start(); lfo.start(); this.nodes[id] = [n, lfo]
    } else if (freq === 'forest') {
      const n = this.createNoise('pink'); const f = this.ctx.createBiquadFilter(); f.type = 'bandpass'; f.frequency.value = 1200; f.Q.value = 0.8; n.connect(f); f.connect(gainNode); n.start(); this.nodes[id] = [n]
    } else if (freq === 'fireplace') {
      const n = this.createNoise('brown'); const f = this.ctx.createBiquadFilter(); f.type = 'lowpass'; f.frequency.value = 180; const lfo = this.ctx.createOscillator(); const lg = this.ctx.createGain(); lfo.frequency.value = 0.08; lg.gain.value = 40; lfo.connect(lg); lg.connect(f.frequency); n.connect(f); f.connect(gainNode); n.start(); lfo.start(); this.nodes[id] = [n, lfo]
    } else if (freq === 'warm_room') {
      const n = this.createNoise('pink'); const lf = this.ctx.createBiquadFilter(); lf.type = 'lowpass'; lf.frequency.value = 250; const mf = this.ctx.createBiquadFilter(); mf.type = 'peaking'; mf.frequency.value = 120; mf.gain.value = 6; mf.Q.value = 0.5; const lfo = this.ctx.createOscillator(); const lg = this.ctx.createGain(); lfo.frequency.value = 0.05; lg.gain.value = 20; lfo.connect(lg); lg.connect(lf.frequency); n.connect(lf); lf.connect(mf); mf.connect(gainNode); n.start(); lfo.start(); this.nodes[id] = [n, lfo]
    } else if (freq === 'fabric') {
      const n = this.createNoise('brown'); const f1 = this.ctx.createBiquadFilter(); f1.type = 'bandpass'; f1.frequency.value = 300; f1.Q.value = 2; const f2 = this.ctx.createBiquadFilter(); f2.type = 'lowpass'; f2.frequency.value = 400; const lfo = this.ctx.createOscillator(); const lg = this.ctx.createGain(); lfo.frequency.value = 0.15; lg.gain.value = 60; lfo.connect(lg); lg.connect(f1.frequency); n.connect(f1); f1.connect(f2); f2.connect(gainNode); n.start(); lfo.start(); this.nodes[id] = [n, lfo]
    } else if (freq === 'protected') {
      const osc = this.ctx.createOscillator(); osc.type = 'sine'; osc.frequency.value = 55; const og = this.ctx.createGain(); og.gain.value = 0.04; osc.connect(og); og.connect(gainNode); const n = this.createNoise('brown'); const f = this.ctx.createBiquadFilter(); f.type = 'lowpass'; f.frequency.value = 80; const ng = this.ctx.createGain(); ng.gain.value = 0.08; n.connect(f); f.connect(ng); ng.connect(gainNode); osc.start(); n.start(); this.nodes[id] = [osc, n]
    }
  }
  ...
  stop(id) { if (this.gainNodes[id]) this.gainNodes[id].gain.setTargetAtTime(0, this.ctx.currentTime, 0.5) }
  stopAll() { Object.keys(this.gainNodes).forEach(id => this.stop(id)) }
}
...
function ComfortSounds() {
  ...
  useEffect(() => {
    return () => { if (engine) engine.stopAll(); clearTimeout(tRef.current); clearInterval(cRef.current) }
  }, [])
  ...
  const toggleSound = (sound, volume) => {
    ...
    setActiveSounds(prev => {
      const next = { ...prev }
      if (next[sound.id] !== undefined) { engine?.stop(sound.id); delete next[sound.id] }
      else { if (Object.keys(next).length >= 3) return prev; engine?.play(sound.id, sound.freq, sound.variant, vol); next[sound.id] = vol }
      setIsPlaying(Object.keys(next).length > 0)
      return next
    })
  }

  const updateSoundVolume = (id, vol) => { engine?.setSoundVolume(id, vol); setActiveSounds(prev => ({ ...prev, [id]: vol })) }

  const applyPreset = (preset) => {
    if (engine) engine.stopAll(); setActiveSounds({}); setActivePreset(preset.id)
    setTimeout(() => {
      const newActive = {}
      preset.sounds.forEach(sid => {
        const sound = SOUND_GROUPS.flatMap(g => g.sounds).find(s => s.id === sid)
        if (sound) { const vol = preset.volumes[sid] || 0.5; engine?.play(sound.id, sound.freq, sound.variant, vol); newActive[sid] = vol }
      })
      setActiveSounds(newActive); setIsPlaying(Object.keys(newActive).length > 0)
    }, 100)
  }

  const stopAll = () => {
    engine?.stopAll(); setActiveSounds({}); setIsPlaying(false); setActivePreset(null)
    clearTimeout(tRef.current); clearInterval(cRef.current); setTimeLeft(null); setTimerIdx(null)
  }

  const setTimer = (idx) => {
    clearTimeout(tRef.current); clearInterval(cRef.current); setTimerIdx(idx)
    const timer = TIMERS[idx]
    if (!timer.ms) { setTimeLeft('sleep'); return }
    setTimeLeft(timer.ms / 1000)
    cRef.current = setInterval(() => setTimeLeft(prev => { if (prev <= 1) { clearInterval(cRef.current); stopAll(); return null } return prev - 1 }), 1000)
    tRef.current = setTimeout(stopAll, timer.ms)
  }

  const formatTimeLeft = (secs) => { if (!secs || secs === 'sleep') return ''; const m = Math.floor(secs / 60); const s = secs % 60; return m + ':' + String(s).padStart(2, '0') }
  const handleMasterVol = (val) => { setMasterVol(val); engine?.setMasterVolume(val) }
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                ...
                <p className="text-m" style={{ fontSize: 11 }}>{isPlaying ? activeCount + ' sound' + (activeCount > 1 ? 's' : '') + ' playing' + (timeLeft && timeLeft !== 'sleep' ? ' · ' + formatTimeLeft(timeLeft) : timeLeft === 'sleep' ? ' · sleep mode' : '') : 'tap a preset to begin'}</p>
              </div>
            </div>
            <div style={{ display: 'flex', gap: 8 }}>
              {isPlaying && <button onClick={stopAll} style={{ padding: '6px 12px', background: 'rgba(192,57,43,0.08)', border: 'none', borderRadius: 999, fontSize: 12, color: '#c0392b', cursor: 'pointer' }}>stop</button>}
              <button onClick={() => setShowSounds(p => !p)} style={{ padding: '6px 12px', background: 'rgba(0,0,0,0.05)', border: 'none', borderRadius: 999, fontSize: 12, cursor: 'pointer' }} className="text-s">{showSounds ? 'less' : 'browse'}</button>
            </div>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 14 }}>
            {PRESETS.map(preset => (
              <button key={preset.id} onClick={() => applyPreset(preset)} style={{ padding: '12px 14px', textAlign: 'left', background: activePreset === preset.id ? 'rgba(138,158,140,0.15)' : 'rgba(0,0,0,0.03)', border: activePreset === preset.id ? '1.5px solid rgba(138,158,140,0.4)' : '1.5px solid transparent', borderRadius: 14, cursor: 'pointer', transition: 'all 0.2s ease' }}>
        ...
        {isPlaying && activeCount > 0 && (
          <div style={{ padding: '0 24px 16px', borderTop: '1px solid rgba(0,0,0,0.05)' }}>
            ...
            <div style={{ marginBottom: 10 }}>
              ...
              <input type="range" min="0" max="1" step="0.01" value={masterVol} onChange={e => handleMasterVol(parseFloat(e.target.value))} style={{ width: '100%', accentColor: '#8a9e8c', height: 4, cursor: 'pointer' }} />
            </div>
            {Object.entries(activeSounds).map(([sid, vol]) => {
              ...
              return (
                <div key={sid} style={{ marginBottom: 8 }}>
                  ...
                  <input type="range" min="0" max="1" step="0.01" value={vol} onChange={e => updateSoundVolume(sid, parseFloat(e.target.value))} style={{ width: '100%', accentColor: '#8a9e8c', height: 4, cursor: 'pointer' }} />
                </div>
              )
            })}
          </div>
        ...
        {isPlaying && (
          <div style={{ padding: '0 24px 16px' }}>
            ...
            <div style={{ display: 'flex', gap: 6 }}>
              {TIMERS.map((t, i) => (
                <button key={i} onClick={() => timerIdx === i ? (clearTimeout(tRef.current), clearInterval(cRef.current), setTimerIdx(null), setTimeLeft(null)) : setTimer(i)}
                  style={{ flex: 1, padding: '7px 4px', background: timerIdx === i ? '#2c2c2e' : 'rgba(0,0,0,0.04)', color: timerIdx === i ? '#faf9f7' : undefined, border: 'none', borderRadius: 10, fontSize: 11, cursor: 'pointer', transition: 'all 0.2s ease' }} className={timerIdx === i ? '' : 'text-s'}>
                  {t.label}
                </button>
              ))}
            </div>
          ...
          <div style={{ borderTop: '1px solid rgba(0,0,0,0.05)', padding: '16px 24px 20px' }}>
            ...
            {SOUND_GROUPS.map(group => (
              <div key={group.id} style={{ marginBottom: 16 }}>
                <p className="section-label" style={{ fontSize: 10, letterSpacing: '0.08em', textTransform: 'uppercase', marginBottom: 8 }}>{group.icon} {group.label}</p>
                  ...
                  {group.sounds.map(sound => {
                    ...
                    return (
                      <button key={sound.id} onClick={() => !isDisabled && toggleSound(sound)}
                        style={{ padding: '8px 14px', borderRadius: 999, border: 'none', cursor: isDisabled ? 'default' : 'pointer', background: isActive ? '#8a9e8c' : 'rgba(0,0,0,0.05)', color: isActive ? '#ffffff' : undefined, opacity: isDisabled ? 0.4 : 1, fontSize: 13, transition: 'all 0.2s ease', display: 'flex', alignItems: 'center', gap: 5 }} className={isActive ? '' : 'text-s'}>
            ...
            {activeCount >= 3 && <p className="text-m" style={{ fontSize: 11, textAlign: 'center', marginTop: 4 }}>Maximum 3 sounds. Turn one off to add another.</p>}
          </div>
        )}
      </div>
    </section>
...
export default function ComfortPage() {
  ...
  const reflect = async () => {
    ...
    setTimeout(async () => {
      ...
      const btn = document.querySelector('[data-reflect-btn]')
      if (btn) { rewardFloat(btn, '◦', '#8a9e8c'); rewardRipple(btn, '#8a9e8c') }
    }, 900)
  }
  ...
  const startBreathe = () => {
    ...
    const btn = document.querySelector('[data-breathe-btn]')
    if (btn) rewardFloat(btn, '◎', '#8a9e8c')
    const cycle = () => {
      setPhase('breathe in...')
      const t1 = setTimeout(() => {
        setPhase('hold...')
        ...
        <section style={{ marginBottom: 20 }}>
          <div className="card" style={{ borderRadius: 20, padding: '26px' }}>
            ...
            <button
              ...
              style={{ marginTop: 14, padding: '11px 28px', background: text.trim() && !loading ? '#2c2c2e' : '#f0ede8', color: text.trim() && !loading ? '#faf9f7' : '#b0a99a', border: 'none', borderRadius: 999, fontSize: 14, cursor: text.trim() && !loading ? 'pointer' : 'default', transition: 'all 0.2s ease' }}>
        ...
        <section style={{ marginBottom: 20 }}>
          <div className="card" style={{ padding: '32px 24px', borderRadius: 20, textAlign: 'center' }}>
            ...
            <div
              ...
              style={{ width: 120, height: 120, borderRadius: '50%', margin: '0 auto 20px', cursor: 'pointer', position: 'relative', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
              {[0,1,2].map(i => (
