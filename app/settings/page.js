...
import { sanitizeDisplayName, sanitizeFeedback } from '../../lib/sanitize'
...
export default function SettingsPage() {
    ...
    window.google.accounts.id.initialize({
      ...
      callback: async (response) => {
        try {
          ...
          if (!res.ok) { setError('Authentication failed. Please try again.'); return }
          const decoded = await res.json()
          if (decoded.error) { setError('Authentication failed. Please try again.'); return }
  ...
  const handleSaveUsername = async () => {
    const clean = sanitizeDisplayName(username)
    if (!clean) return
    const updated = updateUser({ displayName: clean })
    setUser(updated)
    setUsername(clean)
    await pushToSupabase(updated)
  ...
  return (
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 20px 100px', background: 'var(--bg-primary)', minHeight: '100vh' }}>
      <Navigation />
          ...
          {user.isGuest ? (
            showOptions ? (
              <div>
                ...
                <button onClick={handleGoogleSignIn} style={{ width: '100%', padding: '13px 20px', background: 'var(--bg-card)', border: '1.5px solid rgba(0,0,0,0.12)', borderRadius: 12, fontSize: 14, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 10, marginBottom: 10, color: 'var(--text-primary)' }}>
            ...
            <div>
              ...
              {showLogoutConfirm ? (
                <div style={{ padding: '14px', background: 'var(--bg-muted)', borderRadius: 12 }}>
          ...
          <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: 18, boxShadow: 'var(--shadow)' }}>
            {editingProfile ? (
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                ...
                <button onClick={() => setEditingProfile(true)} style={{ padding: '8px 16px', background: 'var(--bg-muted)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 999, fontSize: 13, color: '#6b6b6e', cursor: 'pointer' }}>edit</button>
              </div>
            )}
          </div>
        </section>
        ...
        <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: '6px 18px', boxShadow: 'var(--shadow)' }}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', padding: '16px 0' }}>
            <div style={{ flex: 1, paddingRight: 16 }}>
              ...
              <p style={{ fontSize: 12, color: '#a0a0a3', lineHeight: 1.5 }}>Go invisible. Shows as "Disappeared" to your circle.</p>
            </div>
...
function FeedbackSection({ user }) {
  ...
  const handleSend = async () => {
    ...
    const lastSent = localStorage.getItem('stillspace_lastFeedback')
    if (lastSent && Date.now() - parseInt(lastSent) < 60000) {
      const secsLeft = Math.ceil((60000 - (Date.now() - parseInt(lastSent))) / 1000)
      setError('Please wait ' + secsLeft + ' seconds before sending again.')
      return
    }

    const clean = sanitizeFeedback(message)
    if (!clean) return
    ...
    localStorage.setItem('stillspace_lastFeedback', Date.now().toString())
    setSending(false)
    setSent(true)
    setMessage('')
      ...
      <div style={{ background: 'var(--bg-card)', borderRadius: 16, padding: 18, boxShadow: 'var(--shadow)' }}>
        {!open ? (
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
            ...
            <button onClick={() => setOpen(true)} style={{ padding: '8px 16px', background: 'var(--bg-muted)', border: '1px solid rgba(0,0,0,0.08)', borderRadius: 999, fontSize: 13, color: '#6b6b6e', cursor: 'pointer' }}>
              open
            </button>
          </div>
        ) : sent ? (
          ...
          <div>
            <p style={{ fontSize: 14, color: '#6b6b6e', marginBottom: 12, lineHeight: 1.6 }}>What's on your mind? Be as honest as you like.</p>
            {error && (
            ...
            <textarea
              ...
              rows={4}
              style={{ width: '100%', padding: '12px 16px', background: 'var(--bg-input)', border: '1.5px solid transparent', borderRadius: 12, fontSize: 14, color: 'var(--text-primary)', resize: 'none', outline: 'none', fontFamily: 'var(--font-sans)', lineHeight: 1.6, transition: 'border-color 0.2s ease', marginBottom: 6 }}
              onFocus={e => e.target.style.borderColor = '#8a9e8c'}
              onBlur={e => e.target.style.borderColor = 'transparent'}
            />
            <p style={{ fontSize: 11, color: '#c0bdb8', textAlign: 'right', marginBottom: 10 }}>{message.length}/2000</p>
            <div style={{ display: 'flex', gap: 8 }}>
              ...
              <button onClick={() => { setOpen(false); setMessage(''); setError(null) }} style={{ padding: '10px 16px', background: 'none', border: '1px solid rgba(0,0,0,0.1)', borderRadius: 10, fontSize: 13, color: '#a0a0a3', cursor: 'pointer' }}>
                cancel
              </button>
            </div>
          </div>
