...
import { initUser, setStatus as saveStatus } from '../../lib/store'
...
export default function StatusPage() {
  ...
  const handleSave = () => {
    if (!selected) return
    const updated = saveStatus(selected)
    setUser(updated)
    setSaved(true)
    setChanging(false)
    ...
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 20px 100px', background: '#faf9f7', minHeight: '100vh' }}>
      ...
      {user.currentStatus && !changing && (
        <div style={{
          marginBottom: 24,
          padding: '20px',
          background: currentStatus ? currentStatus.bg : '#f4f3f0',
          borderRadius: 16,
          display: 'flex',
          alignItems: 'center',
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
            <span style={{
              ...
              background: currentStatus ? currentStatus.color : '#a0a0a3',
              flexShrink: 0,
            }} />
            <div>
              <p style={{ fontSize: 11, color: '#a0a0a3', letterSpacing: '0.06em', textTransform: 'uppercase', marginBottom: 3 }}>current status</p>
              <p style={{ fontSize: 15, color: currentStatus ? currentStatus.color : '#6b6b6e' }}>
      ...
      {changing && (
        <div>
          {user.currentStatus && (
            <p style={{ fontSize: 13, color: '#a0a0a3', marginBottom: 16 }}>
              Choose a new status to replace your current one.
            </p>
          )}
            ...
            {STATUSES.map(status => (
              ...
              >
                <span style={{
                  ...
                  boxShadow: selected === status.id ? '0 0 0 3px ' + status.color + '22' : 'none',
                  transition: 'box-shadow 0.2s ease',
                }} />
                <span style={{ fontSize: 15, color: selected === status.id ? status.color : '#6b6b6e' }}>
                  {status.label}
                </span>
          ...
          <div style={{ display: 'flex', gap: 10 }}>
            <button
              ...
              style={{
                flex: 1,
                padding: '14px',
                background: saved ? '#8a9e8c' : selected ? '#2c2c2e' : '#f4f3f0',
                color: selected ? '#faf9f7' : '#a0a0a3',
                border: 'none',
                borderRadius: '999px',
                fontSize: 15,
      ...
      {user.statusHistory && user.statusHistory.length > 0 && (
        <section style={{ marginTop: 36 }}>
          <p style={{ fontSize: 12, color: '#a0a0a3', marginBottom: 14, letterSpacing: '0.06em', textTransform: 'uppercase' }}>
