...
export default function StatusPage() {
  ...
  const handleCancel = () => {
    ...
    setCustomText(user.customStatusText || '')
  }

  const handleClearCustom = async () => {
    const updated = updateUser({ customStatusText: '' })
    ...
    <div style={{ maxWidth: 480, margin: '0 auto', padding: '0 20px 100px', background: '#faf9f7', minHeight: '100vh' }}>
      ...
      {(user.currentStatus || hasCustom) && !changing && (
        <div style={{
          ...
          background: hasCustom ? '#f5f0eb' : (currentStatus ? currentStatus.bg : '#f4f3f0'),
          borderRadius: 16,
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 10, flex: 1 }}>
            <span style={{
              ...
              background: hasCustom ? '#a09080' : (currentStatus ? currentStatus.color : '#a0a0a3'),
            }} />
            <div>
              ...
              {hasCustom && <p style={{ fontSize: 11, color: '#b0a99a', marginTop: 2 }}>custom status</p>}
            </div>
          </div>
          ...
          {!useCustom ? (
            <>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 8, marginBottom: 20 }}>
                {STATUSES.map(status => (
                  <button
                    ...
                    style={{
                      padding: '14px 18px',
                      background: selected === status.id ? status.bg : '#ffffff',
                      border: selected === status.id ? '1.5px solid ' + status.color + '44' : '1px solid rgba(0,0,0,0.06)',
                      borderRadius: 16, cursor: 'pointer', textAlign: 'left',
                      display: 'flex', alignItems: 'center', gap: 12,
                      transition: 'all 0.25s ease',
                      boxShadow: selected === status.id ? 'none' : '0 2px 20px rgba(0,0,0,0.04)',
                  ...
                  >
                    ...
                    {user.currentStatus === status.id && !hasCustom && (
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  onClick={handleSave}
                  disabled={!selected}
                  style={{ flex: 1, padding: '14px', background: saved ? '#8a9e8c' : selected ? '#2c2c2e' : '#f4f3f0', color: selected ? '#faf9f7' : '#a0a0a3', border: 'none', borderRadius: '999px', fontSize: 15, cursor: selected ? 'pointer' : 'default', transition: 'all 0.3s ease', letterSpacing: '0.03em' }}
                >
                  {saved ? 'status updated' : (user.currentStatus || hasCustom) ? 'update status' : 'share with circle'}
                </button>
                {(user.currentStatus || hasCustom) && (
                  <button onClick={handleCancel} style={{ padding: '14px 20px', background: 'none', border: '1px solid rgba(0,0,0,0.1)', borderRadius: '999px', fontSize: 14, cursor: 'pointer', color: '#a0a0a3' }}>
                    cancel
                  </button>
                )}
              </div>
            ...
            <div>
              <p style={{ fontSize: 14, color: '#6b6b6e', marginBottom: 12, lineHeight: 1.6 }}>
                Write how you feel in your own words. Max 50 characters.
              </p>
              <div style={{ position: 'relative', marginBottom: 8 }}>
                <textarea
                  ...
                  rows={3}
                  style={{ width: '100%', padding: '14px 18px', background: '#ffffff', border: '1.5px solid transparent', borderRadius: 16, fontSize: 15, color: '#2c2c2e', resize: 'none', outline: 'none', fontFamily: 'var(--font-sans)', lineHeight: 1.6, boxShadow: '0 2px 20px rgba(0,0,0,0.05)', transition: 'border-color 0.2s ease' }}
                  onFocus={e => e.target.style.borderColor = '#8a9e8c'}
                  onBlur={e => e.target.style.borderColor = 'transparent'}
                />
                <p style={{ position: 'absolute', bottom: 10, right: 14, fontSize: 11, color: customText.length > 40 ? '#c0392b' : '#c0bdb8' }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button
                  ...
                  style={{ flex: 1, padding: '14px', background: customSaved ? '#8a9e8c' : customText.trim() ? '#2c2c2e' : '#f4f3f0', color: customText.trim() ? '#faf9f7' : '#a0a0a3', border: 'none', borderRadius: '999px', fontSize: 15, cursor: customText.trim() ? 'pointer' : 'default', transition: 'all 0.3s ease' }}
                >
