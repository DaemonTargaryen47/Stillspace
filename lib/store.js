...
export const setStatus = async (statusId) => {
  ...
  const updated = updateUser({ currentStatus: statusId, customStatusText: null, statusHistory: history })
...
export const connectByCode = async (myUser, theirCode) => {
  ...
  const newMember = {
    ...
    customStatusText: theirUser.custom_status_text || null,
...
export const getCircleWithLiveStatuses = async (user) => {
  ...
  const { data } = await supabase
    .from('users')
    .select('invite_code, status, custom_status_text, display_name, disappear_mode, chat_toggles')
  ...
  return circle.map(member => {
    ...
    const theirToggles = live.chat_toggles ? JSON.parse(live.chat_toggles) : {}
    return {
      ...member,
      ...
      theirChatToggle: theirToggles[user.inviteCode] || false,
    }
  })
}
